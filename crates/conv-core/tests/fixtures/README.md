...
- `image/{bmp,qoi,png,ico,webp,gif,jpeg}/` — the raster hub (see
  `crates/conv-core/src/formats/image/`): every ordered pair among BMP/QOI/PNG/ICO/WebP (lossless
  only)/GIF/JPEG (baseline only), plus malformed-input cases per format. JPEG is the one lossy
  format in this hub — its golden cases assert structural properties (magic bytes,
  `support::assert_convert_starts_with_magic`), not byte-exact output; see `tests/golden.rs`'s
  JPEG section for why. The harder decoder cases — PNG's real dynamic-Huffman-compressed input and
  every scanline filter type, ICO's real 32-bit raw-DIB entry, WebP's predictor transform/meta-
  prefix groups/heavy backward references/both color-indexing pixel-bundling factors, GIF's real
  interlaced input and full random 233-color palette, JPEG's real 4:2:0-subsampled input and
  restart markers — all from real files built by independent encoders (Pillow/libwebp) — are
  covered as Rust unit tests in `png.rs`/`ico.rs`/`webp.rs`/`gif.rs`/`jpeg.rs` themselves instead
  of fixtures here — see those modules' doc comments for why: they check the codec against
  independently-built input, not the public `conv_core::convert` API this suite is for.
