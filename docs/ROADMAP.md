...
| 🚧 | Image conversion ported to `conv-core` — BMP, QOI, PNG, ICO, WebP (lossless/VP8L), GIF, and JPEG (baseline) landed (see [the format catalog](#format-catalog--target-state)); AVIF and HEIC remain, both needing a real intra-frame video codec (AV1 and HEVC respectively) rather than "just" a DCT/Huffman codec like JPEG — each its own separate, larger undertaking |
