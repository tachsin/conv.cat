...
| 🚧 | Image conversion ported to `conv-core` — BMP, QOI, PNG, ICO, WebP (lossless/VP8L), and GIF landed (see [the format catalog](#format-catalog--target-state)); JPEG and lossy WebP remain, both real DCT-based codecs and each its own separate undertaking |
...
| Images | PNG, JPEG, WebP, AVIF, BMP, GIF, ICO, QOI, HEIC (decode) | 🚧 In progress — 6 of 9 target formats ported (BMP, QOI, PNG, ICO, WebP-lossless, GIF), Phase 2. WebP here means VP8L (lossless) only — lossy WebP (`VP8 `) is a real intra-frame video codec (DCT/WHT, boolean arithmetic coding, block prediction) and out of scope for the same reason JPEG is; see [`vp8l.rs`](../crates/conv-core/src/formats/image/vp8l.rs) module docs |
