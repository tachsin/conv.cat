...
| 🚧 | Image conversion ported to `conv-core` — BMP, QOI, PNG, ICO, and WebP (lossless/VP8L) landed (see [the format catalog](#format-catalog--target-state)); JPEG and lossy WebP remain, both real DCT-based codecs and each its own separate undertaking |
