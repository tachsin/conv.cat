...
| 🚧 | Image conversion ported to `conv-core` — BMP, QOI, PNG, and ICO landed (see [the format catalog](#format-catalog--target-state)); WebP is next (see that entry's note on why it's a bigger step than PNG was) |
...
| Images | PNG, JPEG, WebP, AVIF, BMP, GIF, ICO, QOI, HEIC (decode) | 🚧 In progress — 4 of 9 target formats ported (BMP, QOI, PNG, ICO), Phase 2. WebP next — see [`png.rs`](../crates/conv-core/src/formats/image/png.rs) and [`ico.rs`](../crates/conv-core/src/formats/image/ico.rs) module docs for why it's a bigger step than either of those was (two unrelated codecs, lossless VP8L and lossy VP8, bundled under one container) |
